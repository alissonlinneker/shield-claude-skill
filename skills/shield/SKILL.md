...
Run `${CLAUDE_SKILL_DIR}/../../scripts/check-prereqs.sh` from the skill directory. This checks for:
...
Run `${CLAUDE_SKILL_DIR}/../../scripts/detect-stack.sh` in the target project directory. Outputs JSON:
```json
{
...
1. Run `${CLAUDE_SKILL_DIR}/../../scripts/generate-shannon-config.sh` with detected stack info to produce a Shannon YAML config
2. Copy/symlink the target repo into Shannon's `./repos/` directory
3. Run `${CLAUDE_SKILL_DIR}/../../scripts/run-shannon.sh` with URL and repo name
...
**SAST (Semgrep):**
```bash
${CLAUDE_SKILL_DIR}/../../scripts/run-sast.sh <project-path> <language>
```
Uses language-specific rules from `${CLAUDE_SKILL_DIR}/../../configs/semgrep-rules/`.

**Secrets (gitleaks):**
```bash
${CLAUDE_SKILL_DIR}/../../scripts/run-secrets.sh <project-path>
```
Scans entire repository for hardcoded secrets, API keys, tokens.

**SCA (Package Audit):**
```bash
${CLAUDE_SKILL_DIR}/../../scripts/run-sca.sh <project-path> <package-manager>
```
Runs the appropriate audit command for the detected package manager.

**Dependency Freshness (all modes):**
```bash
${CLAUDE_SKILL_DIR}/../../scripts/run-outdated.sh <project-path> <package-manager>
```
...
Run `${CLAUDE_SKILL_DIR}/../../scripts/consolidate.sh` to merge all tool outputs into a single normalized JSON:
```json
{
  "findings": [
    {
...
Run `${CLAUDE_SKILL_DIR}/../../scripts/calculate-score.sh` on the consolidated JSON.
...
Generate the report using `${CLAUDE_SKILL_DIR}/../../templates/report.md` structure.
...
3. **Create GitHub issues** — One issue per finding using `${CLAUDE_SKILL_DIR}/../../templates/issue.md`
...
Use the template at `${CLAUDE_SKILL_DIR}/../../templates/report.md` for generating security reports.
Use the template at `${CLAUDE_SKILL_DIR}/../../templates/issue.md` for generating GitHub issues.
